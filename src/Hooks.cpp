        ...
        std::jthread([=] {
            if (const auto player{ RE::PlayerCharacter::GetSingleton() }) {
                if (const auto cell{ player->GetParentCell() }) {
                    logger::info("Found parent cell. Sleeping...");
                    std::this_thread::sleep_for(2s);
                    if (player->GetParentCell() && cell == player->GetParentCell()) {
                        logger::info("Cells match. Continuing...");
                        SKSE::GetTaskInterface()->AddTask([=] {
                            ...
                            for (const auto& item : inv | std::views::keys) {
                                if (const auto armo{ item->As<RE::TESObjectARMO>() }) {
                                    ...
                                    if (armo_slot == hands_slot && !hands_worn && !hands_found) {
                                        hands_found = true;
                                        manager->EquipObject(a_this, armo, nullptr, 1, nullptr, true, false, false, false);
                                        logger::debug("{} (0x{:x}): Equipped hands slot item {} (0x{:x}) found in inventory on 3D load", actor_name, actor_form_id, armo_name,
                                                      armo_form_id);
                                    }
                                    if (armo_slot == feet_slot && !feet_worn && !feet_found) {
                                        feet_found = true;
                                        manager->EquipObject(a_this, armo, nullptr, 1, nullptr, true, false, false, false);
                                        logger::debug("{} (0x{:x}): Equipped feet slot item {} (0x{:x}) found in inventory on 3D load", actor_name, actor_form_id, armo_name,
                                                      armo_form_id);
                                    }
                                    if (armo_slot == head_slot && !head_worn && !head_found) {
                                        head_found = true;
                                        manager->EquipObject(a_this, armo, nullptr, 1, nullptr, true, false, false, false);
                                        logger::debug("{} (0x{:x}): Equipped head slot item {} (0x{:x}) found in inventory on 3D load", actor_name, actor_form_id, armo_name,
                                                      armo_form_id);
                                    }
                                    if (armo_slot == body_slot && !body_found) {
                                        body_found = true;
                                        manager->EquipObject(a_this, armo, nullptr, 1, nullptr, true, false, false, false);
                                        logger::debug("{} (0x{:x}): Equipped body slot item {} (0x{:x}) found in inventory on 3D load", actor_name, actor_form_id, armo_name,
                                                      armo_form_id);
                                    }
                                }
                            }
